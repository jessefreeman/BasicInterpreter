...
public class ExceptionTests
{
    ...
    public void TestMaxIterations(string script, string expectedPrintedOutput, BasicInterpreterError expectedErrorType)
    {
        ...
        var expectedErrorMessage = errorMessageTemplate; // No line number replacement needed

        exManager.HandleException(interpreterException);

        // Concatenate the expected printed output with the expected error message
        var fullExpectedOutput = expectedPrintedOutput + expectedErrorMessage;
    ...
    [InlineData("10 LET B$ = \"Hello\"\n20 FOR I = 1 TO 3\n30 PRINT A$\n40 NEXT I",
        30)] // Test undefined variable inside a loop with other defined variables
    ...
    {
        var expectedError = BasicInterpreterError.VariableNotDefined;
        ...
        var errorMessageTemplate = ExceptionManager.ErrorTemplates[expectedError];
        var expectedErrorMessage = errorMessageTemplate.Replace("{line}", expectedLineNumber.ToString());
    ...
    {
        var expectedError = BasicInterpreterError.InvalidTypeAssignment;
    ...
    public void TestVariableErrors(string script, int expectedErrorLineNumber)
    {
        ...
        // Use the template from ExceptionManager
        var errorMessageTemplate = ExceptionManager.ErrorTemplates[BasicInterpreterError.VariableNotDefined];
    ...
    [Theory]
    [InlineData("10 FOR I = 1 TO 10\n20 NEXT J\n30 NEXT I\n", BasicInterpreterError.NextWithoutFor,
        30)] // Incorrect variable in NEXT
    [InlineData("10 FOR I = 1 TO 10\n30 NEXT I, J\n", BasicInterpreterError.NextWithoutFor,
        30)] // Repeated variable in NEXT
    [InlineData("10 LET I = 1\n20 NEXT I\n", BasicInterpreterError.NextWithoutFor,
        20)] // NEXT without corresponding FOR
    [InlineData("10 FOR I = 1 10\n20 NEXT I\n", BasicInterpreterError.ParsingError,
        10)] // Missing TO keyword in FOR loop
    ...
    [InlineData("10 LET A$ = \"Hello\"\n20 PRINT A$\n30 LET A$ = 123\n40 PRINT A$",
        BasicInterpreterError.InvalidTypeAssignment, 30, "Hello\n")]
    [InlineData("10 LET A = 123\n20 PRINT A\n30 LET A = \"Hello\"\n40 PRINT A",
        BasicInterpreterError.InvalidTypeAssignment, 30, "123\n")]
    [InlineData("10 FOR $I = 1 TO 10\n20 PRINT I\n30 NEXT I\n", BasicInterpreterError.ParsingError, 10, "")]
    public void TestLetStatementErrors(string script, BasicInterpreterError expectedErrorType,
        int expectedErrorLineNumber, string expectedPrintedOutput)
    {
        var exception = Record.Exception(() =>
        {
            interpreter.Load(script);
    ...
    [Theory]
    [InlineData("10 FOR I = 1 TO 10\n30 NEXT J\n", BasicInterpreterError.NextWithoutFor,
        30)] // Mismatched NEXT variable
    [InlineData("10 FOR I = 1 TO 10\n20 FOR I = 1 TO 5\n30 NEXT I\n40 NEXT I\n", BasicInterpreterError.NextWithoutFor,
        40)] // Extra NEXT statement
    [InlineData("10 FOR I = 1 TO 10\n20 NEXT I\n30 NEXT I\n", BasicInterpreterError.NextWithoutFor,
        30)] // Extra NEXT statement without corresponding FOR
    [InlineData("10 FOR I = 1 TO 10\n20 PRINT I\n30 FOR J = 1 TO 5\n40 PRINT J\n50 NEXT J\n",
    ...
    [Theory]
    [InlineData("10 PRINT \"Hello\"\n10 PRINT \"World!\"", BasicInterpreterError.DuplicateLineNumber,
        "Duplicate line number at 10")]
    [InlineData("10 PRINT \"Hello\"\n10 LET A = 5\n10 PRINT \"World\"", BasicInterpreterError.DuplicateLineNumber,
        "Duplicate line number at 10")]
    [InlineData("10 FOR I = 1 TO 3\n10 PRINT \"World\"", BasicInterpreterError.DuplicateLineNumber,
        "Duplicate line number at 10")]
    [InlineData("10 PRINT \"Hello\"\n20 LET A = 5\n20 FOR I = 1 TO 3", BasicInterpreterError.DuplicateLineNumber,
        "Duplicate line number at 20")]
    [InlineData("10 PRINT \"Hello\"\n20 PRINT \"World\"\n10 LET A = 5\n20 FOR I = 1 TO 3",
        BasicInterpreterError.DuplicateLineNumber, "Duplicate line number at 10")]
    [InlineData("10 PRINT A\n10 LET A = 5", BasicInterpreterError.DuplicateLineNumber, "Duplicate line number at 10")]
    [InlineData("10 PRINT \"Hello\"\n20 PRINT \"World\"\n20 PRINT \"Again\"", BasicInterpreterError.DuplicateLineNumber,
        "Duplicate line number at 20")]
    [InlineData("10 PRINT \"A\"\n30 PRINT \"B\"\n30 PRINT \"C\"\n40 PRINT \"D\"",
        BasicInterpreterError.DuplicateLineNumber, "Duplicate line number at 30")]
    [InlineData("5 PRINT \"Start\"\n15 PRINT \"Middle\"\n15 PRINT \"Middle Again\"\n25 PRINT \"End\"",
        BasicInterpreterError.DuplicateLineNumber, "Duplicate line number at 15")]
    [InlineData("100 PRINT \"X\"\n200 PRINT \"Y\"\n100 PRINT \"Z\"", BasicInterpreterError.DuplicateLineNumber,
        "Duplicate line number at 100")]
    public void TestDuplicateLineNumberError(string script, BasicInterpreterError expectedErrorType,
    ...
    public void TestDuplicateLineNumberNotFirstLine(string script, int expectedDuplicateLineNumber)
    {
        ...
        // Use the template from ExceptionManager
        var errorMessageTemplate = ExceptionManager.ErrorTemplates[BasicInterpreterError.DuplicateLineNumber];
        var expectedErrorMessage = errorMessageTemplate.Replace("{line}", expectedDuplicateLineNumber.ToString());
    ...
    [InlineData(
        "10 PRINT \"Start\"\n20 FOR A = 1 TO 2\n30 FOR B = 1 TO 2\n40 FOR C = 1 TO 2\n50 PRINT \"Nested\"\n50 PRINT \"Duplicate in deeply nested loop\"",
        50)]
    ...
    public void TestDivisionByZeroError(string script, int expectedErrorLineNumber)
    {
        ...
        // Use the template from ExceptionManager
        var errorMessageTemplate = ExceptionManager.ErrorTemplates[BasicInterpreterError.DivisionByZero];
        var expectedErrorMessage = errorMessageTemplate.Replace("{line}", expectedErrorLineNumber.ToString());
    ...
    public void TestZeroStepValue(string script, BasicInterpreterError expectedErrorType, int expectedErrorLineNumber,
