...
public class ForNextTests
{
    ...
    /// iterating from 1 to 5, but with the loop variable incremented inside the loop, resulting in the odd numbers being printed.
    /// </summary>
    [Theory]
    [InlineData("10 FOR I = 1 TO 5\n20 PRINT I\n30 I = I + 1\n40 NEXT I\n", "1\n3\n5\n")]
    ...
    /// <summary>
    /// Tests the functionality of nested FOR NEXT loops with both loop variables terminated in the same NEXT statement in C64 BASIC.
    /// This test verifies that the interpreter can correctly execute two nested FOR NEXT loops,
    /// iterating the outer loop from 1 to 5, and the inner loop from 1 to 3, printing the combination of I and J.
    /// </summary>
    [Theory]
    ...
    /// This test verifies that the interpreter correctly throws an exception when a NEXT statement is encountered without a corresponding FOR statement,
    ...
    public void TestMissingForStatementInNestedForNextLoops(string script, string expectedOutputBeforeException)
    {
        ...
        string expectedErrorMessage = errorMessageTemplate.Replace("{line}", 40.ToString());

        exManager.HandleException(interpreterException);

        string fullExpectedOutput = expectedErrorMessage;
    ...
    /// This test verifies that the interpreter correctly throws a syntax error exception when a NEXT statement is encountered without a loop variable,
    /// and that the exception type and error message match the expected ones.
    /// </summary>
    [Theory]
    [InlineData("10 FOR I = 1 TO 5\n20 PRINT \"Iteration: \"; I\n30 NEXT REM Syntax Error\n",
        BasicInterpreterError.ParsingError, 30, "Iteration: 1\n")]
    public void TestSyntaxErrorInForNextLoop(string script, BasicInterpreterError expectedErrorType, int expectedErrorLineNumber, string expectedOutputBeforeError)
    {
        ...
        string fullExpectedOutput = expectedOutputBeforeError + expectedErrorMessage;
