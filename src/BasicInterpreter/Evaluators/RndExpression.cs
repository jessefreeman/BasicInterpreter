...
///     The Commodore 64's RND function used a pseudo-random number generator (PRNG) based on a linear congruential
///     generator (LCG).
///     An LCG is a simple type of PRNG that generates a sequence of numbers that approximates the properties of random
///     numbers.
///     The sequence is determined by a linear equation and a seed, which is updated each time a new random number is
///     generated.
///     The linear equation used by the C64's RND function was Xn+1 = (a * Xn + c) mod m, where Xn is the current seed,
///     Xn+1 is the next seed, and a, c, and m are constants. The constants were chosen such that the generator had a long
///     period
///     (the number of values generated before the sequence repeats) and the values appeared random.
///     However, because the generator was deterministic, if you knew the current seed and the constants, you could predict
///     all future values.
///     The RND function allowed you to set a new seed by passing a negative number, which made it possible to generate
///     repeatable sequences of random numbers.
///     This was useful for testing and debugging programs. If you passed a positive number to the RND function, it
///     returned the last generated random number,
...
public class RndExpression : IExpression
{
    ...
    ///     - If a negative number is passed, the method uses the absolute value of the number as a new seed and generates a
    ///     new random number.
